...
## Day 10-25
* **oop面向对象编程**
  * **继承** is-a关系 三种继承关系，最常用的是public继承，跟虚函数搭配最好
  * **组合composition**：has-a:类中有另外的类作为底层容器，可以完全借用底层容器的功能实现自己的功能；构造和析构：container的构造函数首先调用Component的默认构造函数然后在执行自己的；contrainer的析构函数首先执行自己，然后才调用component的析构函数。
  * **委托Delegation**Composition by reference，编译防火墙
