...
...<br>
#endif<br>
* 模板template<typename T>的使用，可以便于在使用类的时候指定类内成员的类型
* 有些成员函数可以在类内直接定义，这样的函数称为inline函数，只能是简单函数，最终的决定权由编译器决定；也有些函数在类内仅进行声明，在类外进行定义
* class类内可以划分为两个部分，私有和公有的:一般数据和小部分函数设置为私有的，使用关键字private:,大部分成员函数设置为公有的public；
* 类的构造函数可以进行多次重载，一般创建在public区，不过有将构造函数创建在私有区的情况，参考*singleton；另外进行重载是有条件的，**构造函数尽量使用列表初始化的方法，尽量类成员的初始化**。
* 类的常量成员函数(不会改变类的数据内容的时候，在用符号()后面，函数体{}添加const，**添加const定义常量成员函数有重要意义**，后续在定义const类型的的类对象时，可以顺利调用其中方法，否则会报错。
* 参数传递: 值传递和引用传递(**尽量使用引用传递的方式**，实际上传递参数的地址，可以考虑使用const前修饰传递常量引用）
* 返回值传递： 值传递和引用传递（**可以的话尽量使用引用传递**）：两种情况
  * 函数的返回结果是放在函数中新创建的地址中，函数传入参数不会改变，因为是在函数中创建的，所以函数调用结束后，在函数中创建的地址就释放了，所以不能对函数结果进行引用传递。
  * 函数返回结果放在传入参数的地址中，没有创建新的地址，此时传递进来的参数发生了改变，不能定义为const，但是由于结果存放的地址不是在函数中创建的，所以可以使用引用传递的方式返回结果。
